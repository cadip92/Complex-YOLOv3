...
def create_modules(module_defs):
    ...
    for module_i, module_def in enumerate(module_defs):
        ...
        # Adding depthwise-seperable convolution
            ...
            kernel_size = int(module_def["size"])
            pad = (kernel_size - 1) // 2
            if "width_mul" in module_def :
                width_mul = float(module_def["width_mul"])         # Adding additional hyper-parameter width_multiplier for mobile_nets
                dep_out_filters = int(round(output_filters[-1] * width_mul)) # output filters = input_filters * width_multiplier
            else : dep_out_filters = output_filters[-1]
            # Depth-wise Convolution
            modules.add_module(f"dep_conv_{module_i}",
                               nn.Conv2d(
                                   in_channels=output_filters[-1],
                                   out_channels=dep_out_filters,
                                   kernel_size=kernel_size,
                                   stride=int(module_def["stride"]),
                                   padding=pad,
                                   bias=not bn,
